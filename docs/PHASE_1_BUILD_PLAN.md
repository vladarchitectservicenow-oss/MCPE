...
Build a production-ready ServiceNow scoped application that securely provisions and governs model context protocol (mcp) server endpoints within servicenow, enabling ai agents to access instance data through governed, auditable connections.. The application must be installable on any Australia-era ServiceNow instance and must pass automated functional tests with zero external runtime dependencies.
